            ...
            for (Map.Entry<String, String> entry : dataPathsToTrigger.entrySet()) {
                ...
                try {
                    ...
                    if (useDeltaEngine) {
                        ...
                        SQSService.getInstance().sendMessage(
                            System.getenv(MapperConstants.ASSET_MAPPING_DONE_QUEUE_URL),
                            doneEvent.toCommandLine(),
                            receivedEvent.getTenantId());
                    } else {
                        SQSService.getInstance()
                            .sendLegacyMessage(entry, receivedEvent, false);
                    }
                } catch (Exception e) {
