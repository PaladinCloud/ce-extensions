...
func handleValue(config *HttpServer) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		log.Printf("fetching networking rules for tenant id [%s] asset id [%s]\n", tenantId, assetId)
		assetNetworkRules, err := config.AssetNetworkRulesClient.GetPortRuleDetails(r.Context(), tenantId, targetType, assetId)
		if err != nil {
			logError("error fetching asset networking rules", err)
			http.Error(w, err.Error(), http.StatusNotFound)
			return
		}

		b, _ := json.Marshal(assetNetworkRules)
		w.Write(b)
	}
}
